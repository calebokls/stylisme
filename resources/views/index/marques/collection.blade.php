 ...
 <!-- Product -->
                 ...
                 <div class="col-sm-6 col-md-4 col-lg-3 p-b-35 isotope-item women">
					...
					<div class="block2">
						<div style="width:220px;height:220px;border-radius:50%;" class="bg-info block2-pic hov-img0 text-center d-flex align-items-center justify-content-center mt-3">
                            ...
                            <a href="{{route('index.show.Marque.Collection',['marque'=>$marque->id,'user'=>$marque->user_id])}}" class="block2-btn flex-c-m stext-103 cl2 size-102 bg0 bor2 hov-btn1 p-lr-15 trans-04 d-inline-block mt-3 pt-2">
                               {{$marque->name}}
                             </a>
                        </div>
					</div>
    ...
    <script type="text/javascript">
        $('document').ready(function(){
            ...
            {
               $(".isotope-grid").show();
               $("#collection-search").hide();
            }else{
               $(".isotope-grid").hide();
               $("#collection-search").show();
            }
            $.ajax({
                 type: 'GET',
                 url: '/index/index-collection-search',
                 data: { 'search': val },
                 success: function(data) {
                   $("#collection-search").html(data);
                }
            })
        })
    ...
    function loadMoreMarqueModel() {
        return new Promise((resolve, reject) => {
            $.ajax({
                type: 'GET',
                url: '/index/scrollindexcollection',
