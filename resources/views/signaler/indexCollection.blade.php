...
<table class="table table-striped">
  ...
  @foreach($signalisations as $signaly)
    <tr>
        ...
        <td><span>{{$signaly->collectionies_users_name}}</span> <span>{{$signaly->collectionies_user_prenom}}</span></td>
        <td>
            <div class="d-flex gap-2 m-100 justify-content-end">
                ...
                <form action="{{route('user.avertir')}}" method="POST">
                  <input type="hidden" name="user_id" value="{{ $signaly->collectionies_users_id }}">
                   @csrf
   ...
   btnConsulter.forEach(btn=>{
    btn.addEventListener('click',function(){
        let signalerId = this.getAttribute('data-validationy-id');
         $.ajax({
            type:'GET',
            url:'/motif/collection/'+signalerId,
            dataType:"json",
            success: function(response){
               let signaler = response.motif;
               document.querySelector('.modal-body').innerText = response.motif[0].motif
            },
